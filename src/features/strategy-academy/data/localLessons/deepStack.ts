...
export const DEEP_STACK_LESSONS: Lesson[] = [
  // ===== local-deep-implied-odds =====
  {
    ...
    duration: '10 min',
    objectives: [
      '计算小对子 set mine 在 100BB 与 500BB 深度下的隐含赔率差异（33:1 对 166:1），并判断深筹下投机牌价值大幅提升',
      '运用隐含赔率公式（当前底池 + 后续街预期收益 : 跟注金额）复算示例中 500BB 深度 CO 跟注 77 的期望收益（0.12 × 500 − 0.88 × 8 ≈ +53BB）',
      '区分深筹码中价值提升的牌（小对子、同花连牌、同花 Ax、双高连牌）与价值降低的牌（顶对顶踢脚、中等对子、弱两对）',
      '识别反向隐含赔率陷阱（弱 Ax 被 dominated、弱同花被更大同花压制），并判断深筹下支撑全下的最低牌力是坚果或接近坚果',
    ],
    content: [
  ...
  // ===== local-deep-suited-connectors =====
  {
    ...
    duration: '9 min',
    objectives: [
      '列举同花连牌在深筹码中价值提升的四个来源（顺子潜力、坚果同花潜力、隐藏性、双重听牌）',
      '给出各位置同花连牌的推荐范围（UTG 仅 54s-T9s、MP 扩展到 43s-JTs、BTN 全部加一间张），并说明其对位置优势的依赖',
      '计算 16 outs 双重听牌的成牌概率（单街约 34%、两条街约 57%），并比较二四法则粗估 64% 与精确值的偏差',
      '判断同花连牌翻后的处理方式（击中强牌保持隐藏性引诱对手下注、击中听牌跟注而非加注、完全 miss 直接弃牌、击中顶对控制底池）',
    ],
    content: [
      { type: 'heading', content: '为什么同花连牌在深筹中这么值钱？' },
      { type: 'text', content: '同花连牌（Suited Connectors，如 76s、T9s）在深筹码中是"高 EV 投机牌"，原因：\n\n• 顺子潜力：可组成 4 种顺子（如 76s 可组 7-8-9-T-J、6-7-8-9-T、5-6-7-8-9、4-5-6-7-8）\n• 同花潜力：可组成坚果同花（同花 A 在你手里则你是坚果）\n• 隐藏性：顺子/同花不易被对手察觉，可赢大底池\n• 双重听牌：flop 经常组成 OESD + 同花听牌（共 15-16 outs）\n\n深筹码中这些潜力被放大——中大牌后可赢全部深筹。' },
    ...
    quiz: [
      ...
      {
        id: 'local-deep-sc-q4',
        question: '同花连牌在翻牌击中 OESD + 同花听牌（16 outs）的成牌概率（两条街）约为？',
        options: ['约 34%', '约 57%', '约 15%', '约 95%'],
        correctIndex: 1,
        explanation: '16 outs 双重听牌：转牌单街约 34%（16/47），翻牌到河牌两条街约 57%（二四法则估算 16×4=64%，高于精确值），是极强的听牌。',
      },
    ],
  },
];
