...
export const MENTAL_LESSONS: Lesson[] = [
  // ===== local-mental-tilt-recognition =====
  {
    ...
    duration: '9 min',
    objectives: [
      '识别 tilt 的三类信号（身体信号如心跳加速与隧道视觉、行为信号如开牌范围放宽与报复性 bluff、心理信号如"把输的赢回来"）',
      '说明 tilt 的本质（情绪系统劫持理性决策系统，感觉理性但实际已失控），并判断"以为我能控制自己"是最致命的认知错误',
      '区分五种 tilt 类型（失利、不公、失控、报复、疲劳）并给出各自的应对（离桌回顾决策、换桌、降级找回掌控感、定时休息）',
      '运用连续 bad beat 后的紧急处理步骤（离桌 5-10 分钟、4-7-8 呼吸法深呼吸 10 次、评估是否还能保持理性决策）',
    ],
    content: [
  ...
  // ===== local-mental-stop-loss =====
  {
    ...
    duration: '8 min',
    objectives: [
      '给出按 buy-in 计的三档止损规则（单 session 3 个、单日 5 个、单周 10 个 buy-in），并判断情绪止损优先于金额止损',
      '说明止损的本质（用预设规则替代情绪决策，强制切断"再来一把就回本"的循环）',
      '识别止损执行的四大心理障碍（再来一把就回本、今天运气差明天会好、我技术更强、止损就是认输）并给出对应反驳',
      '判断触发止损后"破例"继续打是最致命错误，并运用定时器与资金管理表格辅助执行零破例原则',
    ],
    content: [
      ...
      { type: 'example', content: '示例：NL50 玩家的止损规则（买入 = 100BB = $50）\n\n单 session 止损：3 × 100BB = 300BB（$150）\n单日止损：5 × 100BB = 500BB（$250）\n单周止损：10 × 100BB = 1000BB（$500）\n\n场景：周一 session 输 300BB（触发单 session 止损）\n• 正确应对：立即停止周一 session\n• 错误应对：换桌继续打"换换运气"\n\n场景：周一到周三累计输 800BB\n• 评估：接近单周止损（1000BB），情绪如何？\n• 若情绪稳定：周四可继续，但收紧止损到单 session 200BB\n• 若情绪不稳：周四、周五休息，下周重新开始\n\n场景：单 session 输 200BB 但感觉 tilt\n• 正确应对：立即停止（情绪止损优先于金额止损）\n• 错误应对："还没到 300BB，继续打"' },
  ...
  // ===== local-mental-session-management =====
  {
    ...
    duration: '9 min',
    objectives: [
      '说明 Session 管理的核心公式（决策质量 × 手数 = 长期盈利），并判断单 session 不超过 2 小时与定时休息的必要性',
      '列出 Session 前/中/后的关键动作（前：状态评估 1-5 分且低于 3 不打、设定目标；中：每 60 分钟休息 5 分钟；后：记录盈亏与情绪分并复盘关键牌局）',
      '依据好桌子的标志（Players/Flop 40%+、平均底池 80BB+、有 1-2 个明显弱玩家、自己有位置优势）判断桌选质量，并说出 Session 盈利的 80% 来自桌选',
      '区分"关注决策而非结果"与"接受 variance"两种长期心态，并判断复盘时应问"我的决策对吗"而非"我赢了吗"',
    ],
    content: [
