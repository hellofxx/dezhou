...
export const LIMP_LESSONS: Lesson[] = [
  // ===== local-limp-intro =====
  {
    ...
    duration: '8 min',
    objectives: [
      '复述 Limp 局的四个核心特征（多人底池频率高、SPR 3-5 中等偏低、玩家被动少加注、抽水占比相对高）',
      '判断多人底池中顶对顶踢脚（TPTK）为何大幅贬值，并说出把多人底池当单挑打是最致命错误的原因',
      '列举 Limp 局的三大盈利来源（强牌隔离加注、投机牌便宜看翻牌、翻后精准剥削被动玩家）',
      '识别国内低级别 Limp 局的四大常见误区（跟注范围太宽、强牌不隔离、TPTK 大注打光、忽视抽水）',
    ],
    content: [
      { type: 'heading', content: '什么是 Limp 局？' },
      { type: 'text', content: '国内线下俱乐部和线上低级别（NL2-NL25）最常见的结构就是 Limp 局：翻前多人只跟注大盲而不加注，导致底池多人、筹码浅、决策复杂。这与 GTO 教学中假设的"开牌-3-bet-弃牌"单挑场景截然不同。\n\n典型场景：UTG limp、MP limp、CO limp、BTN limp，到你在大盲只需补一个盲注就能看到 5 人底池的翻牌。' },
  ...
  // ===== local-limp-isolate =====
  {
    ...
    duration: '9 min',
    objectives: [
      '说明隔离加注的三大目标（建立底池获取强牌价值、减少对手人数提升胜率、获取翻后 C-Bet 主动权）',
      '给出面对单人/两人/三人 Limp 的隔离加注尺度（4-5BB / 6-7BB / 8-10BB），并说明为何要大于标准 GTO 尺度',
      '区分适合隔离加注的价值范围（TT+、AQ+、KQs）与形态范围（A2s-A5s、56s-9Ts），并识别不适合隔离的中等牌力与垃圾牌',
      '判断连续 3 次隔离都被多人跟注时的调整方向（收紧隔离范围、加大尺度或转用 Limp behind）',
    ],
    content: [
  ...
  // ===== local-limp-multiway =====
  {
    ...
    duration: '10 min',
    objectives: [
      '比较 TPTK 在 3 人底池（胜率约 60%）与 5 人底池（约 35%）的价值差异，判断多人底池必须收紧价值范围',
      '给出多人底池 C-Bet 频率区间（约 30-40%，单挑约 60-70%），并区分应该 C-Bet 与不该 C-Bet 的场景',
      '列出多人底池多条街价值下注的牌力门槛（4 人底池两对起、5+ 人底池三条起、TPTK 只值一条街）',
      '运用"加注即两对+"黄金法则判断国内 Limp 玩家在多人底池加注的含义，并说出听牌隐含赔率在多人底池为何极佳',
    ],
    content: [
