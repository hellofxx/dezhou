...
export const QUIZ_QUESTIONS: PotOddsQuizQuestion[] = [
  ...
  // ─── 以下为平衡题（id 15-19）：正确答案均为否定项，训练"识别 -EV 跟注" ───
  {
    id: 15,
    category: 'odds-judgment',
    scenario: '底池 $40，对手超池下注 $120（3 倍底池）。你有卡顺听牌（4 outs），Turn 发出，只剩一张牌。',
    question: '你应该跟注吗？',
    options: [
      { text: '跟注，中了顺子就是大底池', isCorrect: false, explanation: '所需胜率 = 120/(40+120+120) = 42.9%，而 4 outs 在 turn 后只有约 8.7%（4/46，Rule of 2 估算 4×2=8%）。缺口巨大，"中了赢很多"不能弥补 34 个百分点的差距。' },
      { text: '弃牌，赔率远远不够', isCorrect: true, explanation: '所需胜率 = 120/(40+120+120) = 42.9%。卡顺在 turn 后只剩约 8.7% equity（4/46）。超池下注把价格抬到听牌完全无法承受的水平，这是清晰的弃牌。' },
    ],
  },
  {
    id: 16,
    category: 'odds-judgment',
    scenario: '河牌圈，底池 $100，对手下注 $100（满池）。你只有第三对，没有任何听牌，只能赢对手的诈唬。根据其行动线估计对手诈唬频率约 20%。',
    question: '用底池赔率衡量，这次 bluff catch 应该跟吗？',
    options: [
      { text: '不该跟注，胜率低于所需赔率', isCorrect: true, explanation: '所需胜率 = 100/(100+100+100) = 33.3%。你只能赢诈唬，胜率 ≈ 对手诈唬频率 20%。20% < 33.3%，跟注是 -EV：EV = 0.2×$200 - 0.8×$100 = -$40，应该弃牌。' },
      { text: '跟注，不能被对手随便推走', isCorrect: false, explanation: '"不想被诈唬"不是数学理由。河牌无后续牌，胜率就是对手的诈唬频率 20%，低于所需的 33.3%，长期跟注必然亏损。' },
    ],
  },
  {
    id: 17,
    category: 'implied-odds',
    scenario: '底池 $50，对手下注 $50，下注后对手只剩 $20 筹码。你有同花听牌（9 outs），Flop 发出，本次跟注只看一张 Turn。',
    question: '直接赔率不够，隐含赔率能救回这次跟注吗？',
    options: [
      { text: '能，同花听牌值得追', isCorrect: false, explanation: '隐含赔率的上限是对手剩余筹码。打平需要中牌后额外再赢约 $113（50/0.19 ≈ 263，263-150 = 113），而对手只剩 $20，缺口无法弥补。' },
      { text: '不能，对手筹码太浅，应弃牌', isCorrect: true, explanation: '直接赔率 = 50/(50+50+50) = 33.3%，看一张牌的胜率 ≈ 19%（9/47）。打平需要中牌后额外赢约 $113（50/0.19 ≈ 263，减去现有底池 150），但对手只剩 $20。隐含赔率被浅筹码封死，弃牌。' },
    ],
  },
  {
    id: 18,
    category: 'reverse-implied',
    scenario: '你持有 6♥5♥，翻牌是 K♥9♥9♣（对子面）。底池 $60，对手下注 $60（满池）。',
    question: '你的低同花听牌该继续吗？',
    options: [
      { text: '继续跟注，9 个同花 outs 足够', isCorrect: false, explanation: '名义 9 outs × 4 ≈ 36% 看似够 33.3%，但对子面上部分 outs 是"脏 outs"：对手若持 9x/K9，你成花他成葫芦；6 高同花还可能被更大红心支配。有效 equity 需大幅打折。' },
      { text: '弃牌，反向隐含赔率太差', isCorrect: true, explanation: '所需胜率 = 60/(60+60+60) = 33.3%。名义 9 outs（≈36%）在对子面上要打折：中花仍可能输给葫芦（对手持 9x/K9），6 高同花还可能被更大红心听牌支配，有效 outs 约 6 个（≈24%）。低于所需且中牌也可能付出更多，弃牌。' },
    ],
  },
  {
    id: 19,
    category: 'implied-odds',
    scenario: '底池 $60，一名超紧的岩石型对手下注 $60（满池）。你有卡顺听牌（4 outs），Flop 发出，本次跟注只看一张 Turn。',
    question: '指望隐含赔率跟注卡顺，可行吗？',
    options: [
      { text: '不可行，应该弃牌', isCorrect: true, explanation: '直接赔率 = 60/(60+60+60) = 33.3%，看一张牌的胜率 ≈ 8.5%（4/47）。打平需要中顺后额外再赢约 $526（60/0.085 ≈ 706，减去现有底池 180），而岩石型玩家在明显危险牌面几乎不会大额支付。隐含赔率是幻觉，弃牌。' },
      { text: '可行，卡顺很隐蔽，中了能赢大池', isCorrect: false, explanation: '"隐蔽"不等于"能收到钱"。缺口需要中牌后额外赢约 $526 才能打平，超紧对手不会支付这种规模。对紧手谈隐含赔率要格外保守。' },
    ],
  },
];
