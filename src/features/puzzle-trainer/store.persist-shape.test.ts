...
/**
 * puzzle-trainer store 持久化形状快照测试。
 *
 ...
 */
describe('puzzle-trainer store persisted shape', () => {
  it('持久化键结构与当前快照一致（变化即需 bump version + migrate）', async () => {
    ...
    const { usePuzzleStore } = await import('./store');
    const { partialize } = usePuzzleStore.persist.getOptions();
