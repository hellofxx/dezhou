...
export default function ProgressReplay() {
  ...
  const hasData = topImprovements.length > 0;
                ...
                >
                  ...
                  <div className="w-16 shrink-0">
                    <div className="h-1.5 bg-[var(--walnut-raised)] rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[var(--brass-bright)] rounded-full transition-all"
                        style={{ width: `${Math.min(entry.improvement, 100)}%` }}
                      />
                    </div>
                  </div>
                  {/* 提升数值 */}
                  <div className="flex items-center gap-0.5 text-[var(--poker-success)] shrink-0">
