...
export const STANDARD_LEVEL_3_CHAPTERS: TheoryChapter[] = [
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: 'formula',
        content: '可复算版本（77 挖 Set，概率与赔率口径同 T2 隐含赔率章）：\n跟注 C 入池，翻牌击中 Set 的概率 p = 1 − C(47,2)/C(50,2) = 1 − 1081/1225 ≈ 11.8%（约 12%，即 1-in-8.5，赔率约 7.5:1）。\nEV = p × W − (1 − p) × C，其中 W = 击中后从对手处**净赢**的金额（不含自己已投入的 C）。令 EV ≥ 0 ⇒ W ≥ ((1−p)/p) × C ≈ 7.5 C，即含回本约 8.5 倍投入的总回收才保本。\n代入学费：有效 200bb、跟注 3bb ⇒ 只需命中时对手再付 22.5bb，而他手里还有约 197bb（占其 11%），门槛极易跨过 ⇒ EV > 0。有效 25bb、跟注 3bb ⇒ 对手翻牌后剩余筹码上限仅 22bb < 7.5×3 = 22.5bb，理论天花板已在保本线之下，再叠加"命中也未必收满"的折价（T2 的 15-20 倍经验法则）⇒ EV < 0。\n核心：投机型牌的价值是"深度 ÷ 学费"的函数，而非牌面本身——这正是上面四档分层里同一手 77 随筹码深度跨档移动的数学依据。',
      },
