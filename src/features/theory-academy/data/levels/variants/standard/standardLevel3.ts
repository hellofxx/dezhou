...
export const STANDARD_LEVEL_3_CHAPTERS: TheoryChapter[] = [
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: 'formula',
        content: '可复算版本（77 挖 Set，概率与赔率口径同 T2 隐含赔率章）：\n跟注 C 入池，翻牌击中 Set 的概率 p = 1 − C(47,2)/C(50,2) = 1 − 1081/1225 ≈ 11.8%（约 12%，即 1-in-8.5，赔率约 7.5:1）。\nEV = p × W − (1 − p) × C，其中 W = 击中那手的总净赢（含底池死钱回补，不含自己已投入的 C）。令 EV ≥ 0 ⇒ W ≥ ((1−p)/p) × C ≈ 7.5 C，即含回本约 8.5 倍投入的总回收才保本。\n代入学费：有效 200bb、跟注 3bb ⇒ 保本需净赢 7.5×3 = 22.5bb；底池中已有的死钱（盲注+对手开局共 4.5bb）覆盖其中 4.5bb，故只需命中时对手平均再付 18bb（占其剩余 197bb 约 9%），门槛极易跨过 ⇒ EV > 0。有效 25bb、跟注 3bb ⇒ 对手翻牌后剩余上限仅 22bb，需平均收下其中约 18bb（82%）才勉强触线，再叠加"命中也未必收满"的折价（T2 的 15-20 倍经验法则）⇒ EV < 0。\n核心：投机型牌的价值是"深度 ÷ 学费"的函数，而非牌面本身——这正是上面四档分层里同一手 77 随筹码深度跨档移动的数学依据。',
      },
      ...
      },
      {
        type: 'example',
        content:
          '实例（翻前分层）：BTN 开局 2.5bb，你在 SB。3-bet 范围分成两叠：价值叠 QQ+、AKs/AKo（约 2.5%，领先 BTN 跟注范围）；诈唬叠 A5s、KQs 类（约 4%，选持 A 或 K 阻断牌的组合，让 BTN 更难用 AA/KK 反加、用 Ax 跟注）。跟注范围叠 JJ-22、AQs、87s 等（投机型靠隐含赔率）。这就是 Gap 逻辑的翻前应用：对 BTN 的宽开局，你既要用更硬的牌反加，也要让诈唬 3-bet 的每一手都有阻断依据。',
      },
      {
        type: 'example',
        content:
          '实例（筹码深度改变分层）：浅筹码 25bb——投机档（55、76s）EV 骤降：没有深度兑现 set 与顺子，且翻后常被迫全下无控池能力；成牌档（大对子、AQ+）EV 上升，摊牌价值主导。深筹码 200bb——角色互换：投机档身价翻倍，边缘同花连张从"弃牌"升为"跟注"。同一手 77：25bb 时面对 3bb 开局应 3-bet 或弃牌，200bb 时轻松跟注挖 set。筹码深度是起手牌分层的第二变量。',
      },
      { type: 'heading', content: '主动进池 vs 被动进池' },
      {
        type: 'text',
        content:
          '现代翻前理论的共识：首入底池（无人加注时）应选择加注而非平跟（Limp）。加注保留 Fold Equity、建立主动权、避免多人底池稀释胜率。平跟的问题在于它只有一条赢牌路径（成牌），且向所有人宣告"我的牌不够加注"。3-bet（再加注）逻辑同理：对抗开局加注，用"3-bet 或弃牌"结构通常优于宽平跟（BB 防守除外，因为已投入盲注且关闭行动）。',
      },
      {
        type: 'highlight',
        content: 'Gap Concept 的现代修正：随着位置与筹码深度变化，"跳档"的幅度会伸缩——BTN 对 CO 开局的 3-bet/跟注范围可以很宽，UTG+1 对 UTG 开局则要极紧。原则不变：面对已展示的牌力，你的标准必须上移。',
      },
      {
        type: 'highlight',
    ...
    quiz: [
      ...
      },
      {
        ...
        question: '以下哪手牌在"UTG 开局加注后你位于 CO"的场景下，通常属于 3-bet 范围里的"价值 3-bet"？',
        options: ['55', 'A9s', 'AK', '76s'],
        correctIndex: 2,
        explanation: '面对开局加注，AK/QQ+ 属于价值 3-bet（领先对手跟注范围，且打走其支配牌）；55、76s 属投机跟注（深筹码靠隐含赔率），A9s 边缘。价值与投机在翻前的分层，正是 Gap 逻辑的具体应用。',
      },
    ],
  },
  {
