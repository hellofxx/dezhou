  ...
  chapters: [
    {
      ...
      content: [
        ...
        { type: 'heading', content: 'SB 开池宽度：约 80% 的频率从何而来' },
        {
          type: 'formula',
          content:
            'SB 开池范围包含所有"面对 BB 随机范围至少有 40% 权益"的牌，再加一部分靠翻后位置优势实现的边缘牌：\n\n可开池牌（约 80%）：\n强牌（JJ+/AK/AQ）+ 同花连张 + 同花 Ax + 任意对子 + 大部分同花牌与部分非同花高张\n\n弃牌（约 20%）：\n完全垃圾（72o、83o 等）与阻断价值差的非同花弱牌\n\nSB 补 0.5 即可看 1.5BB 底池（所需胜率 25%），任何牌对 BB 宽防守范围都接近或超过这条线；即使被 BB 3-bet，SB 的弃牌也只损失 1.5BB 的加注额——低频高风险的 3-bet 遭遇，可以由范围宽度带来的高频盈利覆盖。（概念源自：《Modern Poker Theory》Michael Acevedo Ch.3 单挑范围收窄与放宽）',
        },
        {
          type: 'text',
          content:
        ...
        },
        {
          type: 'example',
          content:
            '实例二（范围随对手移动）：同一手 J♠8♠，面对 BB 3-bet 率高达 25% 的激进玩家，你被 3-bet 后 J8s 通常只能弃牌，min-raise 的偷盲 EV 被侵蚀，应考虑收窄开池；面对 3-bet 率仅 8% 的被动 BB，J8s 的翻后同花与顺子潜力能充分实现，值得开池。范围宽度从来不是一个固定数字，而是对对手攻击频率的直接响应。',
        },
        {
          type: 'example',
          content:
            '实例三（翻后实现的关键）：你（BB）持 6♠5♠ 面对 SB min-raise 跟注。翻牌 K♠8♦4♣，SB 下注 1/3 池，你的 65s 只有卡顺听牌（4 张 7）+ 后门花，权益约 20%，低于 25% 的跟注线，标准弃牌。但若翻牌 7♠6♦4♣ 你中了对子带顺子听牌，权益跃升至约 55%，可以跟注甚至过牌加注。范围宽的意义不是"什么牌都跟"，而是"给了你足够多的牌在正确牌面赢钱"。',
        },
        {
          type: 'highlight',
          content:
        ...
        },
        {
          type: 'pro-tip',
          content:
            '单挑范围速查：SB 约开池 80%（顶端 15% 强牌 + 中段同花连张/Ax + 底部垃圾混合），BB 约防守 60%（跟注叠约 35%-45% + 3-bet 叠约 15%-25%）。每 50 手检查一次：你的弃牌率是否被对手的偷盲利润盯上？你的 3-bet 频率是否太低让 SB 无限开池？两个答案决定下一轮的宽度。',
        },
      ],
      quiz: [
        {
        ...
        },
        {
          ...
          question: '面对 3-bet 率高达 25% 的激进 BB，你（SB）应如何调整范围宽度？',
          options: [
            '保持 80% 不变',
            '收窄开池范围，减少被 3-bet 后只能弃牌的边缘牌',
            '增加开池到 100%',
            '完全停止开池',
          ],
          correctIndex: 1,
          explanation: '高频 3-bet 侵蚀偷盲 EV，边缘牌被 3-bet 后只能弃牌。正确调整是收窄范围；完全停止开池过度反应，保持 100% 是自杀。',
        },
        {
    ...
    },
    {
      ...
      content: [
        ...
        },
        {
          type: 'example',
          content:
            '实例：单挑 100BB 深，SB min-raise、BB 跟注，翻牌 K♠9♦4♣。SB 持 A♦A♣（超对，价值）与 A♦5♦（空气+后门花，诈唬）。SB 下注半池 2BB（底池 4BB），价值:诈唬约 2:1——AA 打价值、A5s 代表同花听牌施压。若 SB 的 A5s 选择过牌，它的下注范围就只剩 AA，对手会迅速识别并弃掉一切边缘牌——极化结构被破坏。',
        },
        {
          type: 'example',
          content:
    ...
    },
    {
      ...
      objectives: [
        ...
        '掌握单挑宽范围中挡牌的三重价值：诈唬阻断、价值解封、翻前 3-bet 选择',
      ...
      content: [
        ...
        { type: 'heading', content: '挡牌的三重价值' },
        {
          type: 'text',
          content:
            '第一重：诈唬阻断（Blocking Bluffs）——你持的牌若是对手范围里的强牌，你的诈唬可信度上升，因为对手"声称"的坚果被你证伪。第二重：价值解封（Unblocking）——你持的牌若没有阻断对手的跟注站，你的价值下注更容易被支付（比如不持 A♠ 时对手更可能有同花跟你）。第三重：翻前组合数学——3-bet 范围选择带阻断牌的诈唬（如 A5s 阻断 AA/AK），被 4Bet 时又能干净弃牌。',
        },
        {
          type: 'example',
          content:
        ...
        },
        {
          type: 'example',
          content:
            '实例三（翻前 3-bet 选择）：单挑你（BB）面对 SB 开池，考虑 3-bet 诈唬。持 A♦5♦ 与持 7♦6♦ 的区别：A5s 阻断 AA 与 AK 各一部分（对手范围里的顶级强牌变少，被 4Bet 概率下降），且被 4Bet 后 A5s 可干净弃牌；76s 不阻断任何强牌，被 4Bet 时更常撞上 QQ+。因此 A5s 是更优的 3-bet 诈唬牌。挡牌让"诈唬组合"从笼统的数量变成精确的选择。',
        },
        {
          type: 'formula',
          content:
            '挡牌的定量效果（你持 A♦A♣，三黑桃面）：\n\n对手持坚果同花 A♠X♠ 的组合：\n无挡牌时：A♠ 配其余黑桃共 10 种（13 − 牌面 3 张黑桃 = 剩余 10 张黑桃）\n你持 A♦（不阻断黑桃）：仍 10 种\n你持 A♠（阻断）：A♠ 已在你手里，坚果同花组合数 = 0\n\n对比：持 A♠ 使对手"最强跟注牌"从 10 种骤减到 0——这就是诈唬成功率的数学来源。（概念源自：《The Mathematics of Poker》Bill Chen / Jeremie Ankenman Ch.10 阻断牌与组合计数）',
        },
        {
          type: 'highlight',
          content:
