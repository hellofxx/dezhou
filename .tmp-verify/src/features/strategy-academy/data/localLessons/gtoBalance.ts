...
export const GTO_BALANCE_LESSONS: Lesson[] = [
  // ===== local-gto-vs-exploit =====
  {
    ...
    duration: '10 min',
    objectives: [
      '区分 GTO（目标是不被任何策略剥削）与剥削（目标是最大化 EV）的本质，并判断两者的 EV 权衡（GTO 下限高上限低、剥削相反）',
      '列举应坚守 GTO 的场景（对手未知、强 REG、多人底池、中高级别玩家池、锦标赛泡沫期）',
      '列举应转向剥削的场景（样本 100+ 手、对手有明显倾向、低级别玩家池、线下俱乐部、单挑）',
      '运用偏离幅度公式「可观测偏差 × (1 − 对手调整概率)」判断偏离随对手调整概率递减，并说出偏离的前提是样本足够且漏洞方向明确',
    ],
    content: [
  ...
  // ===== local-when-to-deviate =====
  {
    ...
    duration: '9 min',
    objectives: [
      '列出偏离 GTO 的三项判断标准（样本 50+ 手、偏差显著超基准 20% 以上、单向偏差）',
      '计算对手 Fold to C-Bet 68% 时 1/3 池 C-Bet 的剥削 EV（0.68 × 5.5 − 0.32 × 2 = +3.10BB），并判断应明确偏离 GTO',
      '区分四类偏差的对应偏离方向（弃牌过多→增加 C-Bet 诈唬、跟注过多→纯价值大注、3-bet 过多→收紧开牌用强牌 4-Bet、Fold to 3-bet 过高→增加 3-bet bluff）',
      '判断偏离的风险边界（50 手以下样本不可靠、强对手会反剥削、多人底池更保守），并说出每 50-100 手复查对手统计的必要性',
    ],
    content: [
