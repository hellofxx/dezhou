...
## Authority
- **设计语言唯一来源**：`poker-ui-demo/DESIGN_LANGUAGE.md`（v1.3+），所有视觉决策以其为准
- **Token 强制**：颜色/字号/圆角/阴影必须来自设计文档的 Token 表，禁止硬编码
- **反模式 veto 权**：对违反 DESIGN_LANGUAGE §10.4 反模式清单的 PR/变更有权要求回退
...
## Capabilities
- 设计语言守护（DESIGN_LANGUAGE.md v1.3+ 的唯一权威解释）
...
10. **场景卡** `.scenario-card`（v1.2）— 绿呢微观视角，用于训练题面
11. **推荐项** `.rec-item`（v1.2）— 左侧优先级色条+图标+文本+箭头
12. **模块入口卡** `.module-card`（v1.2）— 主题色图标方块+hover 上浮
13. **周打卡条** `.streak-rail`（v1.2 新增，v1.3 位置调整）— arena 下方（非页面底部）黄铜-苔藓双色横条，骑椭圆下缘 8–14px；手机端隐藏文字标签只保留图标+数字，禁止横向滚动
14. **Live 呼吸点** `.live-dot`（v1.2）— 8px 黄铜脉冲点，标示进行中
...
- **移动 <768px**（v1.3 细化）：
  - 侧边栏隐藏→底部 mobile-nav；触摸目标 ≥44px
  - arena 椭圆 border-radius `50%/28%`，min-height 100px（480px 下 88px），padding `4px 16px 10px`，margin-top -18px
  - arena 内三分区（上 eybrow+h2+motto / 中 两plaque+卡堆+A♠+筹码水平一行 / 下 小字），ELO/daily-progress-chip 隐藏，两 plaque 对称 top:38%
  - streak-rail 从底部上移至 arena 下方，margin-top -14px 骑椭圆下缘；手机端 padding 8px 10px，隐藏文字标签
  - 训练场模块网格强制 2 列（`.grid.grid-cols-2.md\:grid-cols-3 { grid-template-columns:repeat(2,1fr) !important }`）
  - 取消等高拉伸：`grid-auto-rows:auto` + `.panel.h-full { height:auto !important }` + `.panel .mt-auto { margin-top:12px }`
  - panel padding 12px；module-card padding 10px 12px、icon 28px；quick-drill padding 10px 14px、按钮 min-height 32px；图表 canvas max-height 180px
  - action-btn-row 改 2×2 grid，gap 6px，按钮 min-height 44px
  - gap 压缩：gap-5→12px、gap-4→10px、gap-3→8px
  - **特异性规则**：覆盖 Tailwind md:/lg: 断点类的属性时必须加 `!important`（参见 DESIGN_LANGUAGE §10.5）
...
- 禁止双列不等高（仅桌面/平板）；手机端单列必须取消等高避免面板被拉长
- 禁止非牌桌页面大面积使用 felt 绿
- 禁止手机端 streak-rail 置底（必须在 arena 下方首屏可见）
- 禁止手机端训练场模块网格 1 列（必须 2 列）
- 禁止手机端任何横向滚动条
- 禁止媒体查询内覆盖 Tailwind 断点类却不加 `!important`（特异性不足会失效）
