...
- **五级反馈复用**（v1.8 新增）：复用 `DecisionFeedback` 与 `GRADE_DISPLAY_CONFIG`，根据 EV 损失自动评级；禁止自定义评级。反馈样式为牌室化（v1.3.2）：`GRADE_DISPLAY_CONFIG.color` 引用 globals.css `.grade-*` 类，禁止内联 Tailwind 霓虹类（由 `designTokenGuard.test.ts` 守卫）
