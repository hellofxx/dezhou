...
平台已完成全部核心功能模块的开发，覆盖训练核心、学习路径、留存激励、反馈教练、平台能力五大类共 28 个功能模块。v2.1 在 v2.0 基础上完成排组打法逻辑系统性排查与修复（详见 `CHANGELOG.md` v1.8），建立规范的初学者入门训练体系，新增位置渐进解锁、反馈闭环（relatedLessonId）、自适应难度、课程双层门禁、冻结卡碎片系统、进步回放、GTO 偏差检测、学习路径横向推荐等能力；当前版本 v2.3 在此基础上持续迭代完善。
