...
| 更新说明 | v2.4 新增 §8.4 教育 UX 设计约束 — 学习目标可见性、反馈教学化、课程阅读体验、渐进式信息披露、学习路径可视化、模块间教育目的区分、移动端教育体验 |
...
平台功能按职责划分为 **训练核心**、**学习路径**、**留存激励**、**反馈教练**、**平台能力** 五大类，共计 28 个功能模块（产品视角拆分，对应代码层面的 10 个 feature 目录，跨模块系统集中在 progress store）。
...
| 位置渐进解锁 | 基于 preflop ELO 阈值逐步开放位置训练（阈值以 `range-trainer/constants.ts` 的 `POSITION_UNLOCK_THRESHOLDS` 为唯一事实源，按 UTG → HJ → CO → BTN → SB → BB 顺序递增），未解锁位置在 RangeSelector 中以锁定状态展示 |
...
- 基于日期种子算法生成今日任务（每天一种模块类型轮换），完成后跳转对应训练模块
...
> 2026-08 教学结构演进：课时内由“宏观三段（先全部理论 → 再全部示例 → 再全部实战）”升级为**小节式微观闭环**——概念讲解与对应牌例同屏配对（空间邻近原则），示例支持“预测暂停”互动（先猜后揭示，生成效应），实战作为独立形成性阶段收官，降级时提供“返回对应小节复习”的补救闭环。
...
| L7 | 现金桌专项 | 深筹码、多人底池、Straddle、抽水与选桌（单挑策略归属单挑变体课程体系，不在标准课程内） |
...
| 规格项 | 描述 |
|---|---|
| 三段式互动教学 | 每课时包含概念讲解 → 实例演示 → 实践测验的完整学习闭环；课时内采用小节式微观闭环（概念讲解 + 内嵌实例演示同屏 + 预测暂停互动示例 → 综合实战 → 课后测验），实战降级时提供返回对应小节复习的补救闭环 |
...
| 新增课程 | 3Bet 翻后策略、扑克工具指南、线上 vs 线下差异（标准课程）；单挑（Heads-Up）策略课归属单挑变体课程体系，短牌入门课归属短牌变体课程体系，标准课程不含变体内容 |

**验收标准**：
1. 学院主页展示 9 个 Level 节点（8 级，L4 拆为 L4A/L4B 两个子节点）的等级列表与进度环
2. 每个课时支持小节式学习流程（概念讲解 + 内嵌实例演示 + 预测暂停互动示例 → 综合实战 → 课后测验），小节导航可直达，完成状态持久化
...
9. 标准新课程（3Bet 翻后 / 扑克工具指南 / 线上 vs 线下）可正常访问；单挑策略基础课在单挑变体课程体系内访问，标准课程列表中不出现短牌/单挑变体课
...
**交互细节**：
1. **入口与锁定态**：路径卡片位于策略学院（5.16）「学习路径」区；未满足前置条件（§5.25）时显示锁定态，满足后显示 6 模块总进度环，点击进入路径详情页
2. **路径详情页布局**：竖向步骤条展示 6 个模块（Limp 局应对 → Ante/Straddle → 深筹码调整 → 玩家剥削 → GTO 与剥削平衡 → 情绪管理），每模块旁标注「已完成课数 / 总课数」，点击模块展开其课程列表
3. **课程学习闭环**：每课沿用策略学院小节式微观闭环（概念讲解 + 内嵌实例演示 + 预测暂停互动示例 → 综合实战 → 课后测验），完成状态以 ✓ 持久化标记，支持小节导航直达
4. **对手画像训练 Drill**：完成模块 4（玩家剥削）后解锁；8 道判断题以卡片轮播呈现，每题展示对手 VPIP/PFR/AF 统计条与场景描述，用户选择「正确 / 错误」；选项顺序遵循 §5.26（判断题为固定语义对，对/错 位置经题目 id 种子稳定洗牌，避免固定位置作弊）；提交后即时显示五级反馈（best/correct/inaccuracy/wrong/blunder）与正确应对解析，wrong/blunder 级别显示「去复习」链接跳转策略学院对手阅读课程
5. **路径完成**：6 模块 16 课全部完成后顶部进度环填满并显示「路径完成」徽章，同时触发 §5.24 学习路径横向推荐卡片
6. **继续/返回**：路径页顶部提供「返回学院」按钮；存在未完成课程时显示「继续学习」入口，直达最近未完成小节
...
| 规格项 | 描述 |
|---|---|
| Tilt 前兆识别 | 连续答错 ≥3 题时弹出 Dialog "要不要休息一下？"，提供三选项："我知道了"（仅关闭）/ "学习情绪管理"（跳转 `local-mental-tilt-recognition` 课程）/ "休息一下"（返回 Dashboard）；全局渲染覆盖所有训练页面 |
| Session 止损 | 达到每日题量上限（0/50/100/200 四档，0=无限）时禁止开始新训练会话并显示提示卡片；在所有训练页面采用**开局判定**——进入训练页时一次性判定额度，会话进行中额度耗尽不中途拦断（允许当前会话正常走完结算，下次进入再拦）；上限可在设置页调整 |
...
2. 达到每日题量上限后进入训练页面显示止损提示，无法开始新会话；已开始的会话中途达上限不被拦断，可正常完成结算
...
**交互细节**：
1. **触发与展示位置**：当用户在策略学院完成任一学习路径全部课程（含 §5.17 本土低级别盈利路径）后，在路径完成页底部展示「推荐继续学习」区块，推荐卡以流式排布
2. **卡片内容**：每张推荐卡展示关联路径名称、一句话简介、课程数、难度等级（L1-L8 标识或「进阶 / 高级」标签），右上角显示「关联推荐」徽章以区别于系统主推路径
3. **关联逻辑**：推荐数据来自预设关联表（每个路径定义 1-3 条关联路径）；若当前路径无关联定义，则不展示该区块（不渲染空卡片）
4. **跳转交互**：点击卡片跳转目标路径详情页，并在目标页顶部以 Toast 提示「来自 XX 路径的推荐」，同时回写推荐来源用于学习网络可视化
5. **忽略交互**：卡片右上角提供「×」忽略按钮，点击后折叠该推荐区块并记忆本次忽略（不重复弹出），可在设置页「已忽略推荐」中恢复
6. **实时刷新**：前置路径完成状态变更（如新解锁路径）后，再次进入完成页时推荐列表按最新解锁状态刷新；已锁定的关联路径卡片置灰并标注「需先完成前置」
...
**交互细节**：
1. **锁定态展示**：在策略学院「学习路径」区，本土低级别盈利路径卡片显示锁定遮罩 + 🔒 图标，卡片底部状态条显示「需完成 L1-L3 基础课程（进度 X/3）」，进度数随用户完成 L1-L3 实时更新
2. **快捷跳转**：锁定提示内提供「去学习」按钮，点击直达当前**未完成的最低等级**课程（按 L1 → L2 → L3 顺序校验，跳转到第一个未完成 Level 的学院详情页）；该目标由策略学院课程完成状态实时计算，不写死
3. **解锁判定**：基于策略学院持久化的 Level 完成状态实时校验（非缓存快照），L1+L2+L3 全部完成后卡片锁定遮罩自动移除并播放轻量「解锁」动画，无需用户手动刷新
4. **双层门禁一致性**：与策略学院课程双层门禁一致，锁定状态下路径 URL 直接访问被拦截并回退到学院主页，防止 URL 绕过
5. **解锁后引导**：解锁瞬间在卡片位置弹出 Toast「已解锁：本土低级别盈利路径」，引导用户点击进入
...
| Level | 主题 | 核心内容 |
|-------|------|------|
| T1 | 概率论基础 | 组合计数（C(n,k) 通式与推导）、Outs 与 2/4 法则（含误差分析表与 x4 修正）、方差/标准差公式与长期视角 |
| T2 | 期望值与赔率体系 | EV 完整推导链、底池赔率与所需胜率、隐含/反向隐含赔率公式化、Set Mining 精确概率 |
| T3 | 位置与起手牌理论 | 位置价值与权益实现率量化、Gap Concept 现代修正、Sklansky 基本定理、起手牌 EV 分层 |
| T4 | 范围理论 | 范围构建方法论、组合数学与 Blockers 定量应用、范围/坚果优势牌面判定框架 |
| T5 | 博弈论基础 | 纳什均衡、GTO 概念、MDF/Alpha 代数推导、无差别原则、混合策略与节点锁定实操 |
| T6 | 下注理论 | 下注目的（含保本公式）、极化与线性尺度数学依据、SPR 与三街几何尺度推导、阻塞注均衡 |
| T7 | 对手分析理论 | VPIP/PFR/AF/WTSD/3Bet 指标组合解读、玩家类型学与动态漂移、读牌四步法、剥削调整映射 |
| T8 | 扑克心理学 | Tendler 7 型 Tilt 档案、Session A/B/C 档管理、Kelly 准则与资金心理量化、8 项认知偏差清单 |
| T9 | 经典理论综合 | MOP 玩具博弈详解、ICM 递归算法、多人底池胜率表、GTO-剥削统一框架 |

**经典教材对照**：理论内容对照 9 本业界权威教材体系编写，思想复述 + 通用数学表述，不逐字引用原文；相关章节以「（概念源自：XXX 教材 YY 章）」脚注式标注。教材→核心概念→课程落点映射见下表：

| 教材 | 章节 | 核心概念 | 本项目落点 |
|------|------|---------|-----------|
| The Theory of Poker (Sklansky) | Ch.2-3 | Fundamental Theorem / Gap Concept | T3 第 2-3 章 |
| Harrington on Hold'em Vol.1 | Ch.3-5 | EV / Pot Odds / Implied Odds / 位置 | T2 全部章节、T3 第 1 章 |
| The Mathematics of Poker (MOP) | Ch.1-3, 5-10 | 组合计数 / MDF / Alpha / 无差别 / 几何尺度 | T1 第 1-3 章、T5 第 3-4 章、T6 第 2-3 章 |
| Modern Poker Theory (Acevedo) | Ch.1-3 | GTO 基础 / 求解器 / Sizing | T5 第 1-2 章、T6 第 1 章 |
| MSSA | Ch.1-4 | 范围构建 / Blockers | T4 全部章节 |
| The Mental Game of Poker (Tendler) | 全册 | 7 型 Tilt / A-Game | T8 全部章节 |
| Thinking in Bets (Duke) | Ch.1-4 | 结果导向偏差 / 概率思维 | T8 第 4 章 |
| Applications of NLHE (Janda) | Ch.7 | HUD 指标 / 读牌流程 | T7 全部章节 |
...
| 学习目标与难度标签 | 每章可配置学习目标（先行组织者策略），章节页 Header 展示目标卡片与难度徽章（基础/进阶/高级三档） |
| 标记疑难 | 章末小测支持标记疑难题目（幂等切换），为后续复习清单预留数据 |
| 重测入口 | 小测完成后提供"再测一次"按钮（测试效应：多次提取强化记忆），得分取历史最高分 |
| 学习路径地图 | 主页展示 T1-T9 九节点路径图（三态着色：已完成/进行中/锁定），支持节点点击导航 |
| 继续学习入口 | Level 卡片提供显式"继续学习"按钮，快速返回学习断点 |
| 章节切换骨架屏 | 章节间导航展示 150ms 骨架屏过渡，消除视觉跳变 |
...
10. 内容深度：每章至少 2-3 个实战牌例、关键公式含推导过程、至少 1 个反直觉点标注；章末小测保持 3-5 题且覆盖记忆/理解/分析三类题型
11. 章节页展示学习目标卡片（若配置）与难度徽章
12. 小测中可标记/取消标记疑难题目
13. 小测完成后可点击"再测一次"重新作答
14. 主页学习路径地图正确显示三态并可点击导航
15. Level 卡片"继续学习"按钮导航至首个未完成章节

### 5.28 帮助中心教程

**目标**：为新用户提供完整的平台使用指导，降低上手门槛，减少重复性疑问。

**功能范围**：

1. **快速上手路径**：5 步引导条（定位测试 → 策略学院 → 快速训练 → 今日任务 → 成长数据），每步可点击跳转
2. **模块教程文章**：9 篇（平台总览 1 篇 + 8 篇代表性核心功能模块教程，覆盖主要训练与学习模块），统一 4 节结构（概述 / 操作步骤 / 常见误区 / 关联跳转）
3. **系统概念卡片**：6 个（ELO 段位 / Streak 与冻结卡 / SRS 间隔复习 / 五级反馈 / 每日题量上限 / 数据存储）
4. **FAQ**：8 条常见问题（自研轻量折叠，无第三方依赖）
5. **全局入口**：侧边栏导航项、顶栏帮助按钮（移动端 + 桌面端）、设置页「关于」卡片入口
6. **双语支持**：所有文案走 i18n（`help.*` 命名空间），中英对称
7. **数据驱动**：教程内容为纯 i18n key 数据，新增文章无需改组件代码

**约束**：
- 教程文案为面向用户原创提炼，不搬运内部技术文档
- 纯静态页面，无状态持久化，不发射训练事件
- 模块间跳转仅用路由字符串，不产生模块引用
...
> 本节定义平台作为教育工具所特有的 UX 设计约束，与棋牌美学（§8.1-8.3）互补，共同构成完整的用户体验规范。

#### 8.4.1 学习目标可见性

- 用户在首屏（Dashboard）应能感知当日学习目标与进度，不得需要滚动或点击才能发现
- 今日学习目标应以象牙铭牌样式展示，包含：目标描述 + 已完成/总数 + 进度条
- 正确率趋势应以迷你折线图（sparkline）形式展示在 StreakRail 区域，一目了然

#### 8.4.2 反馈教学化

- 答题反馈不仅是"正确/错误"二态判断，必须包含决策逻辑解释
- wrong/blunder 级别必须展示：GTO 推荐决策 vs 用户决策的对比视图 + 差异原因 + 相关课程链接
- 反馈卡片底部必须提供"再看一题"入口（同类型题目巩固学习）
- 所有训练模块的反馈必须接入五级反馈系统，禁止自定义评级

#### 8.4.3 课程阅读体验

- 课程正文行长不超过 65 字符（max-w-prose），保证阅读舒适度
- 课程内容必须结构化：标题编号（§2.1 格式）、要点总结卡、公式独立展示块
- 移动端课程阅读必须提供顶部阅读进度条
- 章末必须提供要点总结（lesson-takeaway 组件），帮助用户回顾关键概念

#### 8.4.4 渐进式信息披露

- 新手用户（totalSessions < 5）Dashboard 不展示完整训练场模块网格，仅展示"今日推荐学习路径"单一卡片
- 训练场模块入口按训练次数逐步解锁：第 1 次仅范围训练，第 3 次解锁赔率计算，第 5 次解锁全部 6 模块
- 用户可主动锁定学习焦点模块，Dashboard 自动聚焦该模块的进度与推荐

#### 8.4.5 学习路径可视化

- 用户完成 Onboarding 后，首次进入 Dashboard 应展示"你的学习路径"时间线（未来 7 天推荐内容）
- 知识图谱节点应展示完成度环（未开始/进行中/已完成三色），让用户感知整体学习进度
- 策略学院课程阶梯应展示当前位置与前后课程的解锁关系

#### 8.4.6 模块间教育目的区分

- 6 大训练模块应在视觉上有所区分，帮助用户快速识别不同学习能力（记忆/计算/策略/分析）
- 模块卡应展示上次训练时间（"X 天前"或"今日已练"），帮助用户规划学习节奏
- 每个模块应有独特的教育目的描述（如范围训练="记忆训练"、赔率="计算训练"、GTO="决策训练"）

#### 8.4.7 移动端教育体验

- 移动端答题反馈使用底部 Sheet（而非 Modal），符合手势操作习惯
- 移动端概念图展示为纵向列表（而非横向节点图），适合竖屏阅读
- 移动端答题按钮触摸目标 ≥44px，保证手指操作精度
- 移动端课程阅读必须提供顶部阅读进度条
...
平台已完成全部核心功能模块的开发，覆盖训练核心、学习路径、留存激励、反馈教练、平台能力五大类共 28 个功能模块。v2.1 在 v2.0 基础上完成排组打法逻辑系统性排查与修复（详见 `CHANGELOG.md` v1.8），建立规范的初学者入门训练体系，新增位置渐进解锁、反馈闭环（relatedLessonId）、自适应难度、课程双层门禁、冻结卡碎片系统、进步回放、GTO 偏差检测、学习路径横向推荐等能力；当前版本 v2.4 在此基础上持续迭代完善。
...
| 平台能力 | 成就系统 / 今日任务 / 排行榜 / PWA 离线 / 国际化（中/英） / 帮助中心 |
