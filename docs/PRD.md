...
- 仪表盘内嵌“今日任务”卡片，不再提供独立页面与一级导航入口（避免与每日谜题形成两套平行打卡心智）
- 基于日期种子算法生成每日挑战任务（每天一种模块类型轮换），完成后跳转对应训练模块
- 卡片同时提供每日谜题入口与当日完成态，与挑战任务统一呈现为“今日任务”
- 连续天数展示统一使用全局连续训练天数（全站唯一 streak 事实源），不再维护独立的“连续完成挑战天数”计数
...
| 规格项 | 描述 |
|---|---|
| 章节式学习 | 每 Level 含 3-4 章，每章 = 概念讲解（含实例分析段落；公式段落视内容需要配置，每个 Level 至少一处，纯定性主题章节不强制）+ 章末小测（3-5 题） |
| 顺序解锁 | T1 默认解锁，Tn 需完成 T(n-1) 全部章节；章节页防 URL 绕过；调试解锁（5.6.6）可旁路 |
| 章末小测驱动进度 | 小测提交即完成该章（幂等），得分记录历史最高分 |
| 章节回访复习 | 已完成章节可自由回访复习：Level 卡片可展开章节列表直达任意章节（含已完成章节，显示完成态与历史最高分）；已完成章节的阅读页提供免重考导航（返回目录/下一章），重考为可选项且得分取历史最高分 |
...
**验收标准**：
1. 侧边栏“研习”分组展示“理论学院”入口，紧邻“策略学院”并列可达（两学院均属学习内容，归入研习分组与牌局复盘同组）；移动端底部导航亦含“理论学院”项
...
7. 4 项理论成就可正常解锁
8. 重复完成同一章不重复计数（幂等）
9. 已完成章节可通过 Level 卡片章节列表回访，阅读页无需重考即可返回目录或进入下一章；重考后记录取历史最高分
