...
> 2026 年 7 月历史归档。当前月份变更请查看 `docs/CHANGELOG.md`。
> 本文件由 `docs/CHANGELOG.md` 按月分割生成，内容不做追溯修改。
...
- **fix(i18n)**：`tilt.continue` 文案修正"继续训练"→"学习情绪管理"（与实际跳转 `local-mental-tilt-recognition` 课程的行为一致），补齐 `tilt.dismiss`（zh/en）（BUG-17）
...
- **本土课门禁对齐**：CourseView 对 LOCAL_LESSONS 改按 LOCAL_TRACK 前置（l1-l3 全完成）放行，不再继承 l7 的 l3+l5 硬门禁（local-mental-tilt-recognition 白名单保留）
...
- **conceptNodes.ts**：同系列 ID 同步修正；不存在的 `l5-discipline` 映射为 `local-mental-tilt-recognition`
...
- **CourseView 双层门禁**：Level 解锁检查 + prerequisite 解锁检查，防止 URL 绕过（`local-mental-tilt-recognition` 例外）
...
- **TiltWarning 三选项**："我知道了"（仅关闭）+"学习情绪管理"（跳转 local-mental-tilt-recognition）+"休息一下"（返回 Dashboard）
...
- **下风期检测**：`checkDownswing` action 取最近 3 天 `accuracyHistory`，判断严格递减则标记 `isDownswing=true`；`DownswingAlert.tsx` 仅在 isDownswing=true 时渲染，提供"查看应对指南"按钮跳转 `local-mental-tilt-recognition` 课程
